...
const Dashboard = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const companyId = 1; // Default company for MVP
            ...
            {stats && (
              <div className="stats-grid">
                ...
                <div className="stat-item">
                  ...
                  <span className="stat-value pending">{stats.pending_review}</span>
                </div>
                <div className="stat-item">
                  <span className="stat-label">Flagged</span>
                  <span className="stat-value flagged">{stats.flagged}</span>
                </div>
                <div className="stat-item">
            ...
            <div className="modal-body">
              <div className="detail-grid">
                <div className="detail-item">
                  ...
                  <span className="detail-value">{selectedRecord.activity_type}</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">Scope:</span>
                  <span className="detail-value">{selectedRecord.scope}</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">Quantity:</span>
                  <span className="detail-value">{selectedRecord.quantity} {selectedRecord.unit}</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">Date:</span>
                  <span className="detail-value">{selectedRecord.activity_date}</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">Source:</span>
                  <span className="detail-value">{selectedRecord.source_system}</span>
                </div>
                <div className="detail-item">
                  <span className="detail-label">Confidence:</span>
                  <span className="detail-value">{(selectedRecord.confidence_score * 100).toFixed(0)}%</span>
                </div>
                {selectedRecord.flag_reason && (
                  <div className="detail-item full-width">
                    <span className="detail-label">Flag Reason:</span>
                    <span className="detail-value">{selectedRecord.flag_reason}</span>
                  </div>
                )}
              </div>
